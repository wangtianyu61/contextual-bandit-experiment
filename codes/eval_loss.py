...
DIR_PATTERN = '../res/cbresults_ms_agree01/'
...
def print_win_ds_for_algo(df, algo_pattern):
    ...
    #return the best parameter for per dataset/algo/name
def load_names(names, cb_type=None, normalize=True, min_actions=None, min_size=None, ty='best', use_cs=False):
    '''
    ty: best, all or raw
    '''
    cache_file = os.path.join('../datasets/cb_eval/rescache{}'.format('_cs' if use_cs else ''),
    ...
    for name in names:
        ...
        loss_file = os.path.join(resdir, 'all_losses.txt')
...
def concatenate_txt(results_dir, loss_file):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--results_dir', default='../res/cbresults_loan_agree01/')
    ...
    loss_file = os.path.join(args.results_dir, 'all_loss.txt')

    if not os.path.exists(loss_file):
        sys.stderr.write('concatenating loss files...')
       
        #combine all the losses into one file seems
        concatenate_txt(args.results_dir, loss_file)
    ...
    if not args.median:
        if args.weighted:
            ...
            def print_results(df, dfbest):
                ...
                print('***** average_ds of best loss across all (algo, lr):', end=' ')
                print(df.groupby('ds').loss.max().mean())
                print()
                print('***** average loss per (algo, lr)')
                
                print(df.groupby(['algo', 'lr']).loss.mean().sort_values())
                print()
                return df.groupby(['algo', 'lr']).loss.mean().sort_values()
    else:
        def print_results(df, dfbest):
...
#
    # remove supervised results
    df = df_raw.copy()
    df_raw.to_csv('vw_loan/loss_alg.csv', index = None)
    # best learning rate per dataset/algo
    dfbest = df_raw.loc[df_raw.groupby(['ds', 'algo']).loss.idxmax()].copy().sample(frac=1)
    df_avg = print_results(df, dfbest)
    df_avg.to_csv('vw_loan/avg_performance.csv')
...
#        print(dfbest.loc[dfbest.groupby('ds').loss.idxmin()])
...
#
#    if not args.interactive:
#        avg_performance = print_results(df, dfbest)
