...
def save_vw_dataset(X, y, did, ds_dir, choice = True):
    ...
    n_classes = max(y) + 1 
    print("check step")
    fname = 'ds_{}_{}'.format(did, n_classes)
...
if __name__ == '__main__':
    ...
    #the following argument used to define apikey for openml but not found OML_API_KEY in config?
    #openml.config.apikey = OML_API_KEY
    openml.config.set_cache_directory('../datasets/cb_eval/omlcache/')

    print('loaded openML')
    #create the position for dataset
    ...
    for did in sorted(dids):
        ...
        try:
            ...
            #[0] means feature, [1] means classes, [2] and [3] means the target class.
            X, y = ds.get_data(target=ds.default_target_attribute)[0:2]
            X, y = shuffle(X, y)
